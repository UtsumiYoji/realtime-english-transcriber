...
### FR-08: マイク入力対応
- **概要:** オーディオ出力（ループバック）に加えて、マイク入力デバイスからの音声キャプチャに対応する
- **詳細:**
  - GUI に「出力デバイス」「マイクデバイス」の2つのデバイス選択ドロップダウンを表示する
  - 各ドロップダウンに「(無効)」オプションを設け、片方のみでも動作可能とする
  - 両方「(無効)」の場合は開始ボタンを無効化する
  - オーディオ出力とマイク入力は独立した音声ストリームとして同時にキャプチャする
  - 各ストリームはソースタグ（`"output"` / `"mic"`）付きで audio_queue に投入する
  - STT ワーカー内でソースごとに独立した VAD インスタンスを使用し、同時発話でも分離処理する
  - 表示時にソースを区別する視覚的表現を行う:
    - アイコンラベル: 出力 → `🔊`、マイク → `🎤`
    - マイク入力のテキストは出力テキストと異なる色で表示する
  - `config.yaml` に `mic_device` フィールドを追加し、デフォルトマイクデバイスを設定可能とする
- **受け入れ基準:**
  - 出力デバイスとマイクデバイスを同時に選択して開始 → 両方の音声が別々に文字起こしされる
  - 表示上でソース（出力/マイク）が色やアイコンで明確に区別できる
  - 同時に発話しても各ソースの文字起こしが混在しない
  - 片方のみ（出力のみ / マイクのみ）でも正常に動作する
  - 両方「(無効)」選択時は開始ボタンが押せない

### FR-09: 日本語文字起こしの有効/無効切替
- **概要:** マルチリンガルモデル使用時に、日本語音声の文字起こしを有効/無効に切り替えられる
- **詳細:**
  - GUI に「日本語文字起こし」チェックボックスを追加する
  - 無効時（デフォルト・従来動作）: 英語以外の音声は検出後スキップされる
  - 有効時: 検出された言語に関係なく、すべての音声を文字起こしする
  - 日本語発言は `JP:` ラベル付きで表示し、英語の `EN:` ラベルと区別する
  - 日本語発言に対しては DeepL 翻訳を行わない（そのまま表示）
  - `config.yaml` に `japanese_transcription_enabled` フィールドを追加してデフォルト値を設定可能とする
  - ファイル保存時もソースラベル・言語ラベルを含むフォーマットで出力する
- **受け入れ基準:**
  - チェックボックス ON → 日本語音声が `JP:` ラベルで表示される
  - チェックボックス OFF → 従来通り英語以外の音声はスキップされる
  - 日本語発言に対して DeepL 翻訳が実行されない
  - 英語専用モデル (`*.en`) 使用時はチェックボックスが無効化される（言語検出不可のため）

### FR-10: 設定リロード
- **概要:** アプリを再起動せずに `config.yaml` の変更を反映できる
- **詳細:**
  - GUI に「⟳ 設定リロード」ボタンを追加する
  - 録音停止中のみリロード可能（録音中はボタンをグレーアウト）
  - リロード時の処理:
    - `config.yaml` を再読み込みし `AppConfig` を更新
    - `whisper_model` / `compute_type` 変更時: `Transcriber` を再生成（次回使用時に lazy load）
    - `deepl_api_key` 変更時: `Translator` を再生成
    - `vad_threshold` / `max_speech_duration` / `min_speech_ms` 変更時: VAD パラメータ更新
    - `translation_enabled` / `japanese_transcription_enabled` 変更時: GUI チェックボックスを同期
  - リロード完了後、システムメッセージ「設定を再読み込みしました」を表示
  - 変更された項目をログに出力する
- **受け入れ基準:**
  - 録音停止中に `config.yaml` を編集してリロードボタンを押す → 新設定が反映される
  - 録音中はリロードボタンが無効化されている
  - リロード後に変更内容がシステムメッセージとして表示される
  - Whisper モデル変更後、次回録音開始時に新モデルが使用される
...
|---|---|---|
| 2026-02-17 | 1.0 | 初版作成 |
| 2026-02-18 | 1.1 | FR-08 マイク入力対応、FR-09 日本語文字起こし切替、FR-10 設定リロードを追加 |
